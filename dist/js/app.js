        ...
        964: function(module) {
            (function(a, b) {
                ...
                function c(a) {
                    ...
                    a.setAttribute("height", a.offsetHeight))), a;
                }
                function d(a, b) {
                ...
                function i(a) {
                    g(a).forEach((function(a) {
                        if (!a.getAttribute(o)) {
                            var b = "img" === a.nodeName.toLowerCase(), c = b ? a.getAttribute("src") : h(a), d = a.getAttribute("data-rjs"), g = !isNaN(parseInt(d, 10));
                            if (null === d) return;
                            g ? e(a, c, d) : f(a, c, d);
                        }
                    }));
                }
                var j = "undefined" != typeof window, k = Math.round(j ? window.devicePixelRatio || 1 : 1), l = /(\.[A-z]{3,4}\/?(\?.*)?)$/, m = /url\(('|")?([^)'"]+)('|")?\)/i, n = "[data-rjs]", o = "data-rjs-processed";
    ...
    (() => {
        ...
        function formFieldsInit(options = {
            ...
            document.body.addEventListener("focusin", (function(e) {
                ...
                if ("INPUT" === targetElement.tagName || "TEXTAREA" === targetElement.tagName || "LABEL" === targetElement.tagName) {
            ...
            if (options.viewPass) document.addEventListener("click", (function(e) {
                ...
                if (targetElement.closest('[class*="__viewpass"]')) {
                    let isPassBtn = targetElement.closest('[class*="__viewpass"]');
                    let inputType = isPassBtn.classList.contains("_viewpass-active") ? "password" : "text";
                    isPassBtn.parentElement.querySelector("input").setAttribute("type", inputType);
                    if ("text" === inputType) isPassBtn.innerHTML = `\n          <svg class="icon-20">\n            <use xlink:href="../img/sprite.svg#open-eye" />\n          </svg>`; else isPassBtn.innerHTML = `\n          <svg class="icon-20">\n            <use xlink:href="../img/sprite.svg#close-eye" />\n          </svg>`;
                    isPassBtn.classList.toggle("_viewpass-active");
                }
            }));
        }
        let formValidate = {
        ...
        const createSliderContent = (data, parentElement, folder) => {
        ...
        if (menuArrow) {
            ...
            document.body.addEventListener("click", (e => {
                if ((e.target === menuArrow || e.target === menuArrowUse || e.target === headerAvatarBlur) && !menuArrow.classList.contains("menu-active")) {
        ...
        const createVideo_topVideosData = [ {
            img: "1",
            duration: "32:18",
            title: "Concert Stadium / Mike Shinoda / Berlin 09.06.2021",
            count: "23 852",
        ...
        const createVideos = (data, parentElement, folder) => {
            data.map((({img, duration, title, count, time}) => {
                const slide = document.createElement("div");
                const slideHref = document.createElement("a");
                slideHref.href = "#";
        ...
        if (container) {
            createVideos(createVideo_topVideosData, container, "top");
            createVideos(createVideo_topVideosData, container, "top");
            createVideos(createVideo_topVideosData, container, "top");
            createVideos(createVideo_topVideosData, container, "top");
            createVideos(createVideo_topVideosData, container, "top");
        ...
        if (videosAside) {
            createVideos(createVideo_topVideosData, videosAside, "top");
            createVideos(createVideo_topVideosData, videosAside, "games");
        }
        const videosReccommend = document.querySelector(".video__recommendations");
        if (videosReccommend) {
            const currentVideos = createVideo_topVideosData.filter(((item, i) => i < 3));
            createVideos(currentVideos, videosReccommend, "top");
        }
        const myChannelVideos = document.querySelectorAll(".my-channel__videos");
        const myChannelVideosFunc = () => {
            if (myChannelVideos) myChannelVideos.forEach(((item, i) => {
                const currentVideos = createVideo_topVideosData.filter(((item, i) => i < 4));
                createVideos(currentVideos, myChannelVideos[i], "top");
            }));
        };
        myChannelVideosFunc();
        var retina_min = __webpack_require__(964);
        ...
        const inputCheck = e => {
            ...
            } else if (e.target.value.length >= 1) closeIcon.style.opacity = "1"; else {
                searchResults.classList.remove("active");
                searchCon.classList.remove("focus");
                closeIcon.style.opacity = "0";
            }
        };
        const util_text = document.querySelector(".earn .copy__text");
        if (util_text) {
            const textValue = util_text.innerHTML;
        ...
        if (complaintItems) complaintItems.forEach(((item, i) => {
            item.addEventListener("click", (() => {
        ...
        if (signupForm) {
            ...
            signupForm.addEventListener("change", (() => isDisabled([ nameInput.value.trim(), emailInput.value.trim(), passwordInput.value.trim(), confirmPasswordInput.value.trim(), checkbox.checked ], submitBtn)));
        }
        ...
        if (signupPopupForm) {
            ...
            signupPopupForm.addEventListener("change", (() => isDisabled([ namePopInput.value.trim(), emailPopInput.value.trim(), passwordPopInput.value.trim(), confirmPasswordPopInput.value.trim(), checkbox.checked ], submitPopBtn)));
        }
        ...
        if (loginForm) {
            ...
            loginForm.addEventListener("change", (() => isDisabled([ emailPopInput.value.trim(), passwordPopInput.value.trim() ], submitPopBtn)));
        }
        function isDisabled(arr, btn) {
            setTimeout((() => {
                const filteredArr = arr.filter((item => item));
                const errors = document.querySelectorAll(".form__error");
                if (filteredArr.length === arr.length && !errors.length) btn.disabled = false; else btn.disabled = true;
            }), 0);
        }
        ...
        if (createChannelForm) {
            ...
            createChannelForm.addEventListener("change", (() => isDisabled([ nameInput.value.trim() ], submitPopBtn)));
        }
        ...
        if (contactsForm) {
            ...
            contactsForm.addEventListener("change", (() => isDisabled([ nameInput.value.trim(), emailInput.value.trim() ], submitPopBtn)));
        }
        ...
        const removeFile = () => {
            ...
            if (UPLOADED_FILES) UPLOADED_FILES.forEach((image => {
            ...
            })); else [...INPUT_FILE.files] = [];
        };
        if (FILES_LIST_CONTAINER) FILES_LIST_CONTAINER.addEventListener("click", (e => {
        ...
        const previewImages = () => {
            FILES_LIST_CONTAINER.innerHTML = "";
            if (FILE_LIST.length > 0) FILE_LIST.forEach(((addedFile, index) => {
        ...
        if (channel) {
            ...
            const addActiveClass = className => {
                blocks.forEach((block => {
                    ...
                    if (block.classList.contains(className)) block.classList.add("active");
                }));
            };
        }
        (() => {
            ...
            Array.from(objects).map((item => {
                ...
                img.onload = () => {
                    ...
                    return "IMG" === item.nodeName ? item.src = item.dataset.src : item.style.backgroundImage = `url(${item.dataset.src})`;
                };
            }));
        })();
        window.addEventListener("load", retina_min);
        window["FLS"] = true;
        isWebp();
        spollers();
        formFieldsInit({
